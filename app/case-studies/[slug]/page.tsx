...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: study ? `${study.client} Digital Marketing Case Study | Strathmark` : "Case Study Not Found",
    description: study?.excerpt,
    openGraph: {
      title: study ? `${study.client} Digital Marketing Case Study | Strathmark` : "Case Study",
      description: study?.excerpt,
      type: "article",
...
export default async function CaseStudyDetail({ params }: PageProps) {
  ...
  const articleJsonLd = {
    ...
    "@type": "Article",
    headline: `${study.client} Digital Marketing Case Study`,
    description: study.excerpt,
      ...
      <header className="relative overflow-hidden border-b border-white/5 bg-gradient-to-b from-strath-navy to-[#0a101d] px-6 pt-32 pb-16 md:pt-40 md:pb-24">
        ...
        <div className="relative max-w-6xl mx-auto">
          ...
          <div className="mt-8 grid grid-cols-1 gap-10 lg:grid-cols-[1.2fr_0.8fr] lg:items-end">
            <div>
              <div className="flex flex-wrap gap-3 text-[15px] font-mono uppercase tracking-[0.24em]">
              ...
              <h1 className="mt-8 font-serif text-4xl font-bold leading-tight text-white md:text-6xl">
                {study.client} Digital Marketing Case Study
              </h1>
            ...
            <div className="border border-white/10 bg-white/[0.03] p-6 md:p-8">
              <p className="text-[15px] font-mono uppercase tracking-[0.24em] text-gold">Engagement snapshot</p>
              <dl className="mt-6 space-y-5">
                <div>
                  <dt className="text-[15px] font-mono uppercase tracking-[0.2em] text-slate-500">Timeframe</dt>
                  <dd className="mt-2 text-white">{study.timeframe}</dd>
                </div>
                <div>
                  <dt className="text-[15px] font-mono uppercase tracking-[0.2em] text-slate-500">Mandate</dt>
                  <dd className="mt-2 text-sm font-light leading-relaxed text-slate-300">{study.mandate}</dd>
                </div>
                <div>
                  <dt className="text-[15px] font-mono uppercase tracking-[0.2em] text-slate-500">Services</dt>
                  <dd className="mt-3 flex flex-wrap gap-2">
                    {study.services.map((service) => (
                      <span key={service} className="border border-white/10 bg-white/5 px-3 py-2 text-[15px] font-mono uppercase tracking-[0.18em] text-slate-300">
                        {service}
                      </span>
                    ))}
                  </dd>
        ...
        <div className="max-w-6xl mx-auto">
          <section className="grid grid-cols-1 gap-4 md:grid-cols-3">
            {study.metrics.map((metric) => (
              <div key={metric.label} className="border border-white/10 bg-white/[0.03] p-6 md:p-7">
                ...
                <p className="mt-3 text-[15px] font-mono uppercase tracking-[0.22em] text-gold">{metric.label}</p>
                <p className="mt-3 text-sm font-light leading-relaxed text-slate-400">{metric.context}</p>
              </div>
            ))}
          </section>
          <p className="mt-5 max-w-4xl text-[15px] leading-relaxed text-slate-400">
            Measurement note: these historical figures describe the digital engagement shown. They are not AI delivery evidence, forecasts, guarantees, or a substitute for establishing a new client baseline.
          </p>

          <section className="mt-16 grid grid-cols-1 gap-8 xl:grid-cols-[0.95fr_1.05fr]">
            <div className="space-y-8">
              <div className="border border-white/10 bg-white/[0.02] p-8">
                <p className="text-[15px] font-mono uppercase tracking-[0.24em] text-gold">The challenge</p>
              ...
              <div className="border border-white/10 bg-[#0c1322] p-8">
                <p className="text-[15px] font-mono uppercase tracking-[0.24em] text-gold">What changed</p>
                <div className="mt-6 space-y-4">
            ...
            <div className="border border-white/10 bg-white/[0.03] p-8 md:p-10">
              <p className="text-[15px] font-mono uppercase tracking-[0.24em] text-gold">Strategic workstreams</p>
          ...
          <section className="mt-16 border border-gold/20 bg-gold/5 p-8 md:p-10">
            <div className="grid grid-cols-1 gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
              <div>
                <p className="text-[15px] font-mono uppercase tracking-[0.24em] text-gold">Why this engagement matters</p>
          ...
          {relatedStudies.length > 0 && (
            <section className="mt-20 border-t border-white/5 pt-16">
              <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                <div>
                  <p className="text-[15px] font-mono uppercase tracking-[0.24em] text-gold">Related work</p>
                ...
                {relatedStudies.map((related) => (
                  <Link href={`/case-studies/${related.slug}`} key={related.slug} className="group block">
                    <article className="flex h-full flex-col border border-white/10 bg-white/[0.02] p-6 transition-colors group-hover:border-gold/30">
                      <p className="text-[15px] font-mono uppercase tracking-[0.22em] text-slate-500">{related.industry}</p>
                      ...
                      <div className="mt-6 space-y-3 border-t border-white/10 pt-6">
                        {related.metrics.slice(0, 2).map((metric) => (
                          <div key={metric.label} className="flex items-center justify-between gap-4">
                            <span className="text-[15px] font-mono uppercase tracking-[0.18em] text-slate-500">{metric.label}</span>
          ...
          <section className="mt-20 border border-white/10 bg-white/[0.02] p-8 text-center md:p-12">
            <h2 className="font-serif text-3xl font-bold text-white">Need an independent view of your digital performance?</h2>
